...
class TestDatabaseExplorer:
    ...
    def test_tree_has_header(self, explorer):
        """Test tree widget has header."""
        assert explorer.tree.headerItem() is not None
        assert explorer.tree.headerItem().text(0) == "Objects"

    def test_load_databases_populates_tree(self, qapp, mock_connection, test_profile):
        """Test databases are loaded and added to tree."""
    ...
    def test_source_text_cleared_on_folder_select(self, explorer):
        """Test source text is cleared when folder selected."""
        explorer.source_text.setText("Some source code")

        db_item = explorer.tree.topLevelItem(0)
        if db_item:
            explorer.on_item_selected(db_item, 0)
            assert explorer.source_text.toPlainText() == ""

    def test_error_handling_on_database_load_failure(self, qapp, mock_connection, test_profile):
        """Test error handling when database load fails."""
        with patch("app.widgets.database_explorer.DatabaseAccessor") as mock_accessor_class:
            mock_accessor = MagicMock()
            mock_accessor_class.return_value = mock_accessor
            mock_accessor.get_databases.side_effect = Exception("Connection failed")

            explorer = DatabaseExplorer(mock_connection, test_profile)
            explorer.accessor = mock_accessor
            qapp.processEvents()  # Execute QTimer callbacks to trigger load_databases
